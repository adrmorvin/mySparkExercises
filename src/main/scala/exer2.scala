 ...
 * */
  ...
  val categoriesDF1 = categoriesDF.withColumn("category_id", categoriesDF.col("category_id").cast("int"))
  //Creo el valor categoriesDF1 en el que asigno que la columna id sea Int para ordenarla después
  val catgr= categoriesDF1.select("category_id","category_name").orderBy(desc("category_id"))
